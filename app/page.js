...
function App() {
  ...
  const [tempPage, setTempPage] = useState({
    nodes: [
      {data: { id: 'Left', label: 'georgia tech', description: '' }},
      {data: { id: 'Right', label: 'penguins', description: '' }},
      {data: { id: 'Main', label: 'cupcakes', description: '' }},
      {data: { id: 'Backward', label: 'old', description: '' }},
      ],
      edges: [
      ],
  });
  ...
  const handleVertexClick = async (pos, topic) => {
    ...
    setTempPage(updatedPage);
  };

  return (
    <Graph data={tempPage} handleClick={handleVertexClick}></Graph>
